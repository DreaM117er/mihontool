...
1. 選項3：完整結構主體只能建立 chapter_1 資料夾。
  - 目的是為了大略處理主流的漫畫種類，像是單行本、同人本、連載的單一章節、畫集等等。
  - 多個章節需要手動變更 `chapter_n` 資料夾名稱（n為章節），然後再將其放入同一個資料夾再做處理即可。
2. 選項4：完整結構封裝能同時處理多個章節的 cbz 封裝，請安心使用。
3. 採用 `image_001.webp` 作為 漫畫封面 `cover` 的原理不用我多提，你有看過漫畫就知道，它是一個非常基本的邏輯。
