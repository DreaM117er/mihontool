...
echo "---" >> "$FULL_STATS_FILE"
# 輸出核心處理信標 (M-系列)
echo "✅ 資料夾: ${counts[mi]} 個 內部章節完整 （folder）" >> "$FULL_STATS_FILE"
echo "✅ 資料夾: ${counts[md]} 個 內部章節封裝完整 （.cbz）" >> "$FULL_STATS_FILE"
echo "---" >> "$FULL_STATS_FILE"
echo "⚠️  資料夾 ${counts[m3]} 個 需做進一步檢查" >> "$FULL_STATS_FILE"
echo "❌ 資料夾 ${counts[mf]} 個 結構/章節不完整/檔案錯誤" >> "$FULL_STATS_FILE"
echo "---" >> "$FULL_STATS_FILE"
echo "📝 資料夾 ${counts[m1]} 個 未經過處理" >> "$FULL_STATS_FILE"
echo "📝 資料夾 ${counts[m4]} 個 需調整結構" >> "$FULL_STATS_FILE"
